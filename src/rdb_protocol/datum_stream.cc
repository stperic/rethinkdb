...
std::map<region_t, store_key_t> active_ranges_to_hints(
    sorting_t sorting, const boost::optional<active_ranges_t> &ranges) {

    if (!ranges) {
        return std::map<region_t, store_key_t>();
    }
...
// RSI: make sure this updates `shards_exhausted`.
// RSI: handle last_key of min and max
raw_stream_t unshard(
    ...
    raw_stream_t items;
    std::vector<std::pair<raw_stream_t *, size_t> > candidates;
    ...
    for (auto &&pair : stream.substreams) {
        ...
        if (it != active_ranges->ranges.end()) {
            ...
            if (ft != it->second.hash_ranges.end()) {
                ...
                candidates.push_back(std::make_pair(&pair.second.stream, 0));
                if (!reversed(sorting)) {
                    ft->second.key_range.left = pair.second.last_key;
                } else {
                    // RSI: make sure that there isn't an off-by-one inclusivity
                    // error when we update this range.
                ...
                if (ft->second.key_range.is_empty()) {
                    // RSI: consider removing this from the map for efficiency.
    ...
    for (auto &&pair : active_ranges->ranges) {
        for (auto &&hash_pair : pair.second.hash_ranges) {
            ...
            // while unsharding.
            if (!hash_pair.second.totally_exhausted()) {
                pseudoshards.emplace_back(sorting, &hash_pair.second, fresh);
            }
        }
    }
    // If all the hash shards are totally exhausted, return early.
    // RSI: should this ever happen, or is it an error?
    ...
    // If there are no active shards, either we need to mark a saturated shard
    // as active or we're exhausted and need to write that to `shards_exhausted_out`.
    if (!seen_active) {
        if (!seen_saturated) {
            // If all the shards are exhausted, they should also all be totally
            // exhausted because unsharding should have been able to consume
            // everything.
            // RSI: make sure this property still makes sense with our sindex
            // bullshit.
            for (auto &&pair : active_ranges->ranges) {
                r_sanity_check(pair.second.totally_exhausted());
            }
            *shards_exhausted_out = true;
        } else {
            if (!reversed(sorting)) {
                for (auto &&pair : active_ranges->ranges) {
                    if (pair.second.state() == range_state_t::SATURATED) {
                        for (auto &&hash_pair : pair.second.hash_ranges) {
                            if (hash_pair.second.state == range_state_t::SATURATED) {
                ...
                for (auto it = active_ranges->ranges.rbegin();
                     it != active_ranges->ranges.rend();
                     ++it) {
                    if (it->second.state() == range_state_t::SATURATED) {
                        for (auto &&hash_pair : it->second.hash_ranges) {
                            if (hash_pair.second.state == range_state_t::SATURATED) {
                                hash_pair.second.state = range_state_t::ACTIVE;
                            }
                        }
...
// RSI: implement this with truncation etc.
/*
...
// RSI: when you issue a read to a subset of ranges, fetch a larger batch from each.

// RSI: make sure that a read never ends in the middle of an sindex, add an
// assert in this file as well.
